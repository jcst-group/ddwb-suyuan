    ...
    <div id="provenances">
        ...
        <div class="content">
            <ul>
                <li class="content-item">
                    ...
                    <span>采收时间</span>
                    <span style="float:right;margin-right:10px">{{dataMsg.harvestingTime}}</span>
                </li>
                <li class="content-item">
                    <img width="18" style="vertical-align:top" src="../assets/images/2.png">
                    <span>采收批次</span>
                    <span style="float:right;margin-right:10px">{{dataMsg.recoveryCode}}</span>
                </li>
                <li class="content-item">
                    <img width="18" style="vertical-align:top" src="../assets/images/9.png">
                    <span>基地名称</span>
                    <span style="float:right;margin-right:10px">{{dataMsg.baseName}}</span>
                </li>
            </ul>
        </div>
        ...
        <div class="content-baogao" v-if="this.detectionResultPic">
            ...
            <div class="img-wrapper">
                <img width="100%" :src="'/api/pictureInfo/show/' + (this.detectionResultPic ? this.detectionResultPic.pictureUrl : '')">
            </div>
        </div>
    </div>
</template>
...
export default {
    ...
    created() {
        ...
        this.$http.get('/api/retro/recovery/getRecovery?retroCode=' + this.retroCode).then(res => {
            if (res.data.success) {
                this.dataMsg = res.data.result
                this.relationId = this.dataMsg.detectionId
                this.detectionPic = this.dataMsg.detectionPic
                this.detectionResultPic = this.dataMsg.detectionResultPic
                if (this.relationId && this.tabDatas.length) {
                    this.$http.get('/api/retro/recovery/getDetectionItem', {
                        params : {
