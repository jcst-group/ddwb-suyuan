<template>
    <div id="provenances">
        ...
        <div class="banner" v-if="this.detectionPic">
            <img :src="'/api/pictureInfo/show/' + (this.detectionPic ? this.detectionPic.pictureUrl : '')" />
        </div>
        <div class="content">
            <ul>
                <li class="content-item">
                    ...
                    <span>种子/种苗入库时间</span>
                    <span style="float:right;margin-right:10px">{{dataMsg.registrationTime}}</span>
                </li>
                <li class="content-item">
                    <img width="18" style="vertical-align:top" src="../assets/images/2.png">
                    <span>种子/种苗批次</span>
                    <span style="float:right;margin-right:10px">{{dataMsg.seedCode}}</span>
                </li>
                <li class="content-item">
                    <img width="18" style="vertical-align:top" src="../assets/images/3.png">
                    <span>供应商名称</span>
                    <span style="float:right;margin-right:10px">{{dataMsg.supplier}}</span>
                </li>
                <li class="content-item">
                    <img width="18" style="vertical-align:top" src="../assets/images/1.png">
                    <span>入场时间</span>
        ...
        <div class="content-baogao" v-if="this.detectionResultPic">
            <div class="baogao-title">
                <img width="15" style="vertical-align:top" src="../assets/images/4.png">
                <span>种源检测报告</span>
            </div>
            <div class="img-wrapper">
                <img :src="'/api/pictureInfo/show/' + (this.detectionResultPic ? this.detectionResultPic.pictureUrl : '')" width="100%">
            </div>
        </div>
    </div>
</template>
...
export default {
    ...
    created() {
        ...
        this.$http.get('/api/retro/seedBatch/getSeedBatch?retroCode=' + this.retroCode).then(res => {
            if (res.data.success) {
                this.dataMsg = res.data.result
                this.relationId = this.dataMsg.seedDetection ? this.dataMsg.seedDetection.id : ''
                this.detectionPic = this.dataMsg.seedDetection ? this.dataMsg.seedDetection.detectionPic : ''
                this.detectionResultPic = this.dataMsg.seedDetection ? this.dataMsg.seedDetection.detectionResultPic : ''
                if (this.relationId && this.tabDatas.length) {
                    this.$http.get('/api/retro/seedBatch/getDetectionItem', {
                        params : {
